...
const viewAllComments = async (req, res) => {
  try {
    const posts = await Post.find({})
      .populate("comments.commentedBy", "username") // Populate commentedBy field in comments
      .populate("comments.replies.repliedBy", "username") // Populate repliedBy field in replies
      .exec();

    // Extract and format all comments with null checks
    const allComments = posts.reduce((accumulator, post) => {
      const comments = post.comments.map((comment) => ({
        _id: comment._id.toString(),
        postId: post._id.toString(),
        commentBody: comment.commentBody,
        // Ensure commentedBy is not null before accessing username
        commentedBy: comment.commentedBy ? comment.commentedBy.username : "Unknown",
        replies: comment.replies.map((reply) => ({
          _id: reply._id.toString(),
          replyBody: reply.replyBody,
          // Ensure repliedBy is not null before accessing username
          repliedBy: reply.repliedBy ? reply.repliedBy.username : "Unknown",
        })),
      }));
      return accumulator.concat(comments);
    }, []);
